...
We adopt CRB as the sensing metric. According to \cite{huCollaborative2025}, the positioning CRB is defined as the inverse of the effective Fisher information matrix (FIM), which can be expressed as 
\begin{align}
...
\end{align}
\end{subequations}
where \eqref{eq:rateConstraint} and \eqref{eq:splittedRateConstraint} denote the minimum rate of per-user constraint and message splitting constraint, \eqref{eq:powerConstraint} denotes the maximum transmit power constraint, \eqref{eq:AAVConstraint}, \eqref{eq:AAVHeightConstraint}, and \eqref{eq:AAVDistanceConstraint} denote the maximum flying distance, fixed flying height, and minimum distance between AAVs, respectively. And \(R_{min}\) is the minimum rate requirement of each UE, \(p_{\max}\) is the maximum transmit power, \(d_{\max}\) is the maximum flying distance, \(H\) is the AAV height, and \(d_{\min}\) is the minimum AAV distance. 
...
        &\mathop{=}-2B\left\Vert \mathbf{F}^{-1} \boldsymbol{f}_{t} \right\Vert^2 \mathop{<} 0,
    \end{align}
